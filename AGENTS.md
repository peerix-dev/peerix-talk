...
├── main.tsx                 # Entry
├── app.tsx                  # App routing
├── index.css                # Tailwind v4 imports, theme tokens (oklch), base styles
├── components/
│   ├── ui/                  # shadcn/ui primitives
│   └── theme-provider.tsx   # Dark/light/system theme context + keyboard shortcut ("d" toggles dark/light)
├── lib/
│   ├── i18n.ts              # i18next initialization (fallback: en, loadPath: /locales/)
│   └── utils.ts             # `cn()` — clsx + tailwind-merge helper
└── views/
    ├── join/                # Pre-join screen (camera preview, user name, etc)
    └── conference/          # Conferencing UI with a chat after join a room
...
- **i18n keys are nested** — use dot notation: `t("conference.microphone.on")`, `t("join.settings")`, etc.
...
Examples:
- `feat(join): add camera preview toggle`
...
3. Use via `const { t } = useTranslation()` → `t("namespace.key")`.
